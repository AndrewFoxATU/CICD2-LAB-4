...
def update_project_put(project_id: int, payload: ProjectCreate, db: Session = Depends(get_db)):
...
def update_project_patch(project_id: int, payload: ProjectUpdate, db: Session = Depends(get_db)):
   proj = db.get(ProjectDB, project_id)
   if not proj:
       raise HTTPException(status_code=404, detail="Project not found")
...
def update_user_put(user_id: int, payload: UserCreate, db: Session = Depends(get_db)):
   user = db.get(UserDB, user_id)
   if not user:
       raise HTTPException(status_code=404, detail="User not found")
   for field, value in payload.model_dump().items():
...
def update_user_patch(user_id: int, payload: UserUpdate, db: Session = Depends(get_db)):
   user = db.get(UserDB, user_id)
   if not user:
       raise HTTPException(status_code=404, detail="User not found")
   update_data = payload.model_dump(exclude_unset=True)
